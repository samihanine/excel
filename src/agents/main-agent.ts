...
import { emailArtefact } from "@/artefacts/email-artefact";
import { documentArtefact } from "@/artefacts/document-artefact";

/**
 * Agent généraliste : le savoir-faire (DAX, rapports, Excel, mails…) vit dans
 * le prompt de chaque outil et artefact. Ici, seulement la méthode de travail.
 */
export const mainAgent = createAgent({
  name: "Analyste",
  description:
    "Un assistant d'analyse qui répond à partir d'un modèle sémantique Power BI et produit des artefacts (dashboards, feuilles Excel, mails, documents).",
  prompt: [
    "Méthode :",
